/**
 * Small dependency-free SHA-256 + HMAC.
 *
 * The handover digest has to be computed identically on an entry-level Android
 * phone that is offline, in the recycler's browser, and on the server. Pulling
 * in a native crypto module would break at least one of those, so this is a
 * compact pure-TS implementation (~1.5 KB minified).
 */
...
/**
 * One-way identifier for a phone number. The raw number stays on the device;
 * the server only ever stores this.
 */
export function hashPhone(phone: string, salt: string): string {
  const normalised = phone.replace(/\D/g, '').slice(-10);
  return hmacSha256Hex(salt, `phone:${normalised}`);
}
